...
class Dangnhap extends CI_Controller
{
    ...
    //         }
    //     } else {
    //         $this->data['title'] = 'Cuong-Long Bookstore - Đăng nhập tài khoản';
    //         $this->data['view'] = 'dangnhap';
    //         $this->load->view('frontend/layout', $this->data);
    //     }
    // }
    ...
    public function dangnhap()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->input->post('token') != $this->session->csrf_token) {
                $this->data['error'] = 'Token is invalid';
                $this->data['title'] = 'Đăng nhập tài khoản';
                $this->data['view'] = 'dangnhap';
                $this->load->view('frontend/layout', $this->data);
            } else {
    ...
    public function dangky()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $config['smtp_user']    = 'vuvancuongiuh@gmail.com'; //sale.smart.store.2019@gmail.com
            ...
            $this->email->to($email);
            $this->email->subject('Hệ thống Cuong-Long Bookstore - Quà thành viên mới');
            $this->email->message('Bạn đã trở thành thành viên mới của cửa hàng Cuong-Long Bookstore, Cửa hàng tặng bạn 1 mã giảm giá giảm 100.000 đ : ' . $tempcoupon . ' , Mã này có giá trị tới ngày ' . $tempdatelimit . '
        ...
        $this->data['title'] = 'Cuong-Long Bookstore - Đăng ký tài khoản';
    ...
    public function forget_password()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $this->email->subject('Hệ thống Cuong-Long Bookstore - Lấy lại mật khẩu');
        ...
        $this->data['title'] = 'Cuong-Long Bookstore - Quên mật khẩu';
    ...
    public function reset_password_new($id)
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($this->Mcustomer->customer_check_id_email($id, $email) != FALSE) {
                ...
                $this->data['title'] = 'Cuong-Long Bookstore- Cập nhật mật khẩu mới';
