...
class Dangnhap extends CI_Controller
{
    ...
    public function dangnhap()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $this->data['title'] = 'Chau Dien Hasaki - Đăng nhập tài khoản';
            $this->data['view'] = 'dangnhap';
            $this->load->view('frontend/layout', $this->data);
        }
    }
    ...
    public function dangky()
    {
        ...
        $this->form_validation->set_rules('password', 'Mật khẩu', 'required|min_length[8]|max_length[32]|check_strong_password');
        ...
        $this->form_validation->set_rules('phone', 'Số điện thoại', 'required|min_length[9]|numeric|is_unique[db_customer.phone]|max_length[11]');
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $newcoupon = array(
                ...
                'description' => 'Mã giảm giá 20.000 đ tự động khi đăng ký thành công',
                'created' => $today,
                'orders' => 0,
                'trash' => 1,
                'status' => 1,
            ...
            $config['smtp_user']    = 'nxc0406@gmail.com'; 
            $config['smtp_pass']    = 'wmnqaobvlgdoqpgb';
            ...
            $this->email->from('nxc0406@gmail.com', 'Chau Dien Hasaki');
            $this->email->to($email);
            $this->email->subject('Hệ thống Chau Dien Hasaki - Quà thành viên mới');
            $this->email->message('Bạn đã trở thành thành viên mới của cửa hàng Chau Dien Hasaki, Cửa hàng tặng bạn 1 mã giảm giá giảm 100.000 đ : ' . $tempcoupon . ' , Mã này có giá trị tới ngày ' . $tempdatelimit . '
        ...
        $this->data['title'] = 'Chau Dien Hasaki - Đăng ký tài khoản';
    ...
    public function forget_password()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $this->email->subject('Hệ thống Chau Dien Hasaki - Lấy lại mật khẩu');
        ...
        $this->data['title'] = 'Chau Dien Hasaki - Quên mật khẩu';
    ...
    public function reset_password_new($id)
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($this->Mcustomer->customer_check_id_email($id, $email) != FALSE) {
                ...
                $this->data['title'] = 'Chau Dien Hasaki- Cập nhật mật khẩu mới';
