...
class Giohang extends CI_Controller
{
    ...
    {
        $this->data['title'] = 'Chau Dien Hasaki - Giỏ hàng của bạn';
    ...
    public function info_order()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if (isset($_POST['dathang'])) {
                ...
                $vnp_Returnurl = "http://localhost/project_codeign/thankyou.html";
                $vnp_TmnCode = "QJ21GXD3"; //Mã website tại VNPAY 
                $vnp_HashSecret = "XELPBLXZPOBNNJGZXTBVHWHPLHLURNBQ"; //Chuỗi bí mật

                $vnp_TxnRef = $myordercode['orderCode']; //Mã đơn hàng. Trong thực tế Merchant cần insert đơn hàng vào DB và gửi mã này sang VNPAY
                $vnp_OrderInfo = 'Thanh Toán VNPAY';
                $vnp_OrderType = 'billpayment';
                $vnp_Amount = $mymoney['money'] * 100;
                $vnp_Locale = 'vn';
                $vnp_BankCode = 'NCB';
                ...
                $districtId = $_POST['DistrictId'];
                $mydata = array(
                    'orderCode' => $myordercode['orderCode'],
                    'customerid' => $idCustomer,
                    'orderdate' => $today,
                    'fullname' => $_POST['name'],
                    'phone' => $_POST['phone'],
            ...
            $this->data['title'] = 'Chau Dien Hasaki - Thông tin đơn hàng';
    ...
    public function thankyou()
    {
        if ($this->session->userdata('info-customer') || $this->session->userdata('sessionKhachHang')) {
            ...
            $this->email->from('vuvancuongiuh@gmail.com', 'Chau Dien Hasaki');
            $list = array($val['email']);
            $this->email->to($list);
            $this->email->subject('Hệ thống Chau Dien Hasaki');
        ...
        $this->data['title'] = 'Chau Dien Hasaki - Kết quả đơn hàng';
